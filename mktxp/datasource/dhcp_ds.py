...
class DHCPMetricsDataSource:
    ...
    def metric_records(router_entry, *, metric_labels=[], add_router_id=True):
        try:
            ...
            dhcp_lease_records = router_entry.api_connection.router_api().get_resource(
                '/ip/dhcp-server/lease').call('print', {'active': ''})
            ...
            if 'expires_after' in metric_labels:
                translation_table['expires_after'] = lambda c: parse_mkt_uptime(
                    c) if c else 0
            if 'active_address' in metric_labels:
                translation_table['active_address'] = lambda c: c if c else ''
            if 'server' in metric_labels:
                translation_table['server'] = lambda c: c if c else 'defconf'
            ...
            print(
                f'Error getting dhcp info from router{router_entry.router_name}@{router_entry.config_entry.hostname}: {exc}')
            return None
